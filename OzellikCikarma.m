...
%    a_mdnG(i)= edges(group_index) + ((median_index - cumfreq(group_index-1)) / frequency(group_index)) * class_width;
    ...
    a_max(i)=max(inamp1) ;
    a_mdn(i)=median(inamp1,'omitnan') ;         
    a_hmn(i)=harmmean(inamp1,'omitnan') ;    a_gmn(i)=geomean(inamp1,'omitnan') ;
    a_mn(i)=mean(inamp1,'omitnan') ;         a_var(i)=var(inamp1,'omitnan') ;
    a_skw(i)=skewness(inamp1) ;     a_krts(i)=kurtosis(inamp1) ;
    a_std(i)=std(inamp1);
    %instantaneous phase
    p_mn(i)=mean(inp1);                         p_var(i)=var(inp1);
...
RF=[Label(x) ,a_max,a_mdn,a_mdnL,a_mdnH, a_std,a_hmn,a_gmn,a_mn,a_var,a_skw,a_krts,p_mn,p_var,p_skw,p_krts,f_mn,f_var,f_skw,f_krts];
%pause(3);
RF=str2double(RF);
writematrix(RF,'RF_040323.csv','Delimiter','tab','WriteMode','append')
AllRF =[AllRF; RF];
end
